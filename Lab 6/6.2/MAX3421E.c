...
void MAXreg_wr(BYTE reg, BYTE val) {
		...
		XSpi_SetSlaveSelect(&SpiInstance,SpiInstance.SlaveSelectMask); //select MAX3421E
		u8 SendBuf[2] = {reg + 2, val}; //write reg + 2 via SPI, write val via SPI
		int Status = XSpi_Transfer(&SpiInstance, SendBuf, NULL, 2);
		if (Status != XST_SUCCESS){					//read return code from SPI peripheral (see Xilinx examples)
			xil_printf("Error in writing to register.\r\n"); //if return code != 0 print an error
		}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
		...
		XSpi_SetSlaveSelect(&SpiInstance,SpiInstance.SlaveSelectMask); //select MAX3421E
		u8 SendBuf[nbytes+1];
		SendBuf[0] = reg+2; //write reg + 2 via SPI,
		for (int i = 1; i< nbytes+1; i++){ //write data[n] via SPI, where n goes from 0 to nbytes-1
			SendBuf[i] = data[i-1];
		}
		int Status = XSpi_Transfer(&SpiInstance, SendBuf,NULL, nbytes + 1);
		if (Status != XST_SUCCESS){//read return code from SPI peripheral (see Xilinx examples)
			xil_printf("Error in multiple-byte write.\r\n");//if return code != 0 print an error
		}
		XSpi_SetSlaveSelect(&SpiInstance,0); //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
...
BYTE MAXreg_rd(BYTE reg) {
		XSpi_SetSlaveSelect(&SpiInstance,SpiInstance.SlaveSelectMask); //select MAX3421E
		u8 SendBuf[2]; //write reg via SPI
		SendBuf[0] = reg;
		//u8 RecvBuf[1]; // read val via SPI
		int Status = XSpi_Transfer(&SpiInstance, SendBuf, SendBuf, 2);
		if (Status != XST_SUCCESS){	//read return code from SPI peripheral (see Xilinx examples)
			xil_printf("Error in reading register.\r\n"); //if return code != 0 print an error
		}
		XSpi_SetSlaveSelect(&SpiInstance,0); //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
		return SendBuf[1]; //return val
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
		...
		XSpi_SetSlaveSelect(&SpiInstance,SpiInstance.SlaveSelectMask); //select MAX3421E
		u8 SendBuf[nbytes + 1]; 				//write reg via SPI
		SendBuf[0] = reg;
		//u8 RecvBuf[nbytes];		//read data[n] from SPI, where n goes from 0 to nbytes-1
		/*for (int i = 0; i< nbytes; i++){ //read data[n] via SPI, where n goes from 0 to nbytes-1
			RecvBuf[i] = data[i];
		}*/
		int Status = XSpi_Transfer(&SpiInstance, SendBuf,SendBuf, nbytes + 1);
		if (Status != XST_SUCCESS){	//read return code from SPI peripheral (see Xilinx examples)
			xil_printf("Error in multiple-byte read.\r\n"); //if return code != 0 print an error
		}
		for (int i = 0; i < nbytes; i++) {
			data[i] = SendBuf[i+1];
		}
		XSpi_SetSlaveSelect(&SpiInstance,0); //deselect MAX3421E
		return (data+nbytes); //return (data + nbytes);
}
